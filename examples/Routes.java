...
// Compile:
//
// javac -cp ../omalibjava.jar Routes.java
//
// Aufruf:
//
// java -classpath .:../omalibjava.jar Routes <oma-file>
//
public class Routes
{
    static double left;
    ...
    public static void main(String[] args) throws IOException
    {
        ...
        Filter f1 = new TagFilter("name","Köln");
        ...
        r.setFilter(new AndFilter(f4, new TypeFilter('C'), new TagFilter("type","route"), new BlockSliceFilter("route","bus")));

        List<MemberFilter> routes = new ArrayList<>();
        while (true)
        {
            Collection c = (Collection)r.next();
            if (c==null) break;
            routes.add(new MemberFilter(c));
        }

        r.setFilter(new AndFilter(f4,new OrFilter(routes.toArray(new Filter[routes.size()])),new TypeFilter("W")));
        ...
        System.out.println("1 0.75 0.75 c\n");
